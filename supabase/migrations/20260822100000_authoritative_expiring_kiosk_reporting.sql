-- Forward-only replacement of the effective reports RPC. Expiring-Kiosk
-- counts and rows come from the same filtered SQL relation, settings value,
-- and Vietnam calendar date. Clients must not patch these values separately.
  ...
  if report_type = 'overview' then
      ...
      where (p_customer_id is null or k.customer_id = p_customer_id)
        ...
        and (
          ...
          or (lower(p_kiosk_status) = 'expiring_soon'
            and lower(k.status) in ('active', 'warning')
    ...
    kiosk_summary as (
      select
        ...
        count(*) filter (
          where lower(status) in ('active', 'warning')
      ...
      from (
        select
          ...
          case
            ...
            when lower(k.status) in ('active', 'warning')
        ...
        where lower(k.status) in ('pending', 'expired')
          or (lower(k.status) in ('active', 'warning')
            and k.end_date between today_date and today_date + warning_days)
            ...
            where fk.id = i.kiosk_id
              and (
                (lower(p_kiosk_status) = 'expiring_soon'
                  and lower(fk.status) in ('active', 'warning')
