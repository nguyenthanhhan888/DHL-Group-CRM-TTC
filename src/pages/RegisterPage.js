...
export function RegisterPage() {
  ...
  return `
    ${PageHeader({
      ...
      description: 'Gửi thông tin khách hàng và một hoặc nhiều Kiosk để Ban quản trị xác nhận.',
    })}
    <div id="register-official-community">${OfficialCommunityCard({ id: 'register-community' })}</div>
    <section class="form-card registration-card">
      <form id="public-registration-form" novalidate>
        ...
        <div class="form-row">
          ${field('Tên khách hàng / Người liên hệ', 'register-contact-name', { required: true, autocomplete: 'name' })}
          ${field('Tên Facebook hiển thị', 'register-facebook-name', { required: true })}
        </div>
        <div class="form-row">
...
RegisterPage.afterRender = async function afterRenderRegister() {
  ...
  try {
    ...
    const community = document.getElementById('register-official-community');
    if (community) community.innerHTML = OfficialCommunityCard({ id: 'register-community' });
  } catch {
    // Public contact information is optional for form submission.
  }
  await loadOptions();
};
  ...
  return `
    <section class="form-card public-kiosk-card" data-register-kiosk="${id}">
      ...
      <div class="form-row">
        ...
        ${field('Giảm giá (VNĐ)', '', { type: 'number', min: '0', step: '1000', value: '0', data: 'data-kiosk-discount' })}
      </div>
...
function calculateCard(card) {
  ...
  const discount = Number(value(card, 'discount') || 0);
  let total = 0;
...
function validateForm() {
  ...
  cards.forEach((card, index) => {
    ...
    valid = setInlineError(discount, Number(discount.value || 0) >= 0 && Number(discount.value || 0) <= subtotal
      ? '' : 'Giảm giá không hợp lệ hoặc lớn hơn tạm tính.') && valid;
    valid = setInlineError(discountReason, Number(discount.value || 0) === 0 || discountReason.value.trim()
...
function readKiosk(card) {
  return {
    ...
    discount: Number(value(card, 'discount') || 0),
