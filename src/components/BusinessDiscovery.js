...
export function renderBusinessSuggestions(target, query, categories, businessTypes) {
  ...
  target.innerHTML = `<p class="business-suggestion-heading">Bạn có thể đang tìm:</p>${suggestions.map(({ businessType, category }) => `<button type="button" class="business-suggestion" data-select-business="${escapeHtml(businessType.id)}"><span class="business-suggestion-icon">${renderIcon('store')}</span><span class="business-suggestion-copy"><strong>${escapeHtml(businessType.name)}</strong><small>Danh mục: ${escapeHtml(category.name || 'Chưa xác định')}</small></span><span class="business-suggestion-action"><b>${formatCurrency(businessType.price_per_month)}/tháng</b><em>Chọn ${renderIcon('chevron-right')}</em></span></button>`).join('')}`;
}

export function renderBusinessSelection(target, category, businessType) {
  target.classList.remove('hidden');
  target.innerHTML = `<div class="business-selection-primary"><span>Ngành nghề đã chọn</span><strong>${escapeHtml(businessType?.name || '—')}</strong><small>${escapeHtml(category?.name || '—')}</small></div><button type="button" data-change-business>Đổi lựa chọn</button>`;
}
