const { displayOrderStatus, refreshOrder } = require('../payos/_lifecycle');
...
module.exports = async function renewalStatus(req, res) {
  ...
  try {
    ...
    const rows = await rest(config, `/rest/v1/payos_orders?select=*,payments!inner(kiosk_id,months,bonus_months,start_date,end_date,payment_status,kiosks(facebook_name,end_date))&order_code=eq.${orderCode}&payments.kiosk_id=eq.${auth.kid}&limit=1`);
    let row = rows[0]; if (!row) return sendError(res, 404, 'ORDER_NOT_FOUND', 'Không tìm thấy thanh toán phù hợp.');
    const payment = row.payments;
    if (auth.pid && Number(row.payment_id) !== auth.pid) return sendError(res, 403, 'TOKEN_SCOPE_MISMATCH', 'Token không thuộc thanh toán này.');
    row = { ...row, ...await refreshOrder(row) };
