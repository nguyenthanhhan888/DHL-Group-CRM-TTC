...
module.exports = async function publicKioskLookup(req, res) {
  ...
  try {
    ...
    const rows = await rest(config, `/rest/v1/registered_kiosks?select=id,facebook_name,start_date,end_date,status,categories(name),business_types(name,price_per_month)&customer_id=eq.${customers[0].id}&order=facebook_name.asc`);
