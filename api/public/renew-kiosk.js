const { prepareCheckoutRetry } = require('../payos/_lifecycle');
...
const { renewalNonceHash, verifyRenewalToken, issueRenewalToken } = require('./_renewal-token');
...
module.exports = async function publicRenewKiosk(req, res) {
  ...
  try {
    ...
    let months = Number(parsed.value.months);
    if (!ALLOWED_MONTHS.has(months)) return sendError(res, 400, 'INVALID_DURATION', 'Thời hạn gia hạn không được hỗ trợ.');
    const promotionCode = String(parsed.value.promotionCode || '').trim().toUpperCase() || null;
    const prepared = authorization.pid
      ? await callSupabaseRpc('resume_public_kiosk_renewal', { payment_id_input: authorization.pid, kiosk_id_input: authorization.kid }, { serviceRole: true })
      : await callSupabaseRpc('prepare_public_kiosk_renewal', { kiosk_id_input: authorization.kid, months_input: months, nonce_hash_input: renewalNonceHash(authorization.nonce), promotion_code_input: promotionCode }, { serviceRole: true });
    ...
    const checkoutToken = issueRenewalToken({ kioskId: authorization.kid, paymentId: Number(payment.id) });
    const bonusMonths = Number(payment.bonus_months || 0);
    const kioskPeriod = await readKioskPeriod(authorization.kid);
    const period = publicRenewalPeriod(kioskPeriod.currentExpiry, months + bonusMonths);
    const existingOrder = await prepareCheckoutRetry(payment.id);
    if (existingOrder) return res.status(200).json({ checkoutToken, bonusMonths, promotion: payment.promotion_snapshot || null, ...formatRenewalResponse({ order: existingOrder, payment, kiosk: prepared.kiosk_name, months, kioskPeriod, period, reused: true }) });
...
function publicRenewalError(error) { if (['P0001','PAYMENT_REVIEW_REQUIRED','PAYOS_STATUS_UNAVAILABLE','PAYOS_AWAITING_WEBHOOK','CHECKOUT_IN_PROGRESS','PAYMENT_ALREADY_COMPLETED'].includes(error?.code)) return error.message; if(['INVALID_RENEWAL_TOKEN','TOKEN_EXPIRED','TOKEN_SCOPE_MISMATCH'].includes(error?.code))return error.message; if(error?.code==='23505')return'Bạn đang có một mã thanh toán còn hiệu lực.'; if(error?.code==='MISSING_ENV')return'Hệ thống thanh toán chưa được cấu hình đầy đủ.'; return'Không thể tạo thanh toán gia hạn. Vui lòng thử lại hoặc liên hệ hỗ trợ.'; }
